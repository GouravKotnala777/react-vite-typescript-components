...
function Navbar3({blobCurvature="2px", navItems, blobSize="44px", blobY="middle", blobTop="-70px"}:Navbar3PropTypes) {
    ...
    function func(e:MouseEvent<HTMLDivElement>){
        const dataNav = (e.target as HTMLElement).getAttribute("data-nav");
            ...
            >
                {
                    navItems.map(({icon, text}) => (
                        ...
                        >
                            ...
                            ></div>
                            <div data-nav={text} className="absolute top-0 left-0 inset-0 text-center">{icon}</div>
                        </div>
                    ))
                }

            </div>
            <div className="cccc absolute left-0 w-full h-full"
                style={{
                    top:blobTop
                }}
            >
